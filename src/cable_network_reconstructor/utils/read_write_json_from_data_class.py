...
def write_json_statistics(statistics: Statistics):
    ...
    with open("results//statistics.json", "w") as outfile:
        ...
        json_object = dict(
            total_tests_number=statistics.total_tests_number,
            success_percentage=statistics.success_percentage,
...
def write_json_test_cases(model_name, test_case: TestCase):
    ...
    with open(f"results//{model_name}.json", "w") as outfile:
        ...
        outfile.write(
            str(json_object)
            .replace("'", '"')
            .replace("TestComparison(", "")
            .replace(")", "")
            .replace("reference_nodes", '{ "reference_nodes"')
            .replace(', "processing_time"', '}, "processing_time"')
            .replace("=", ":")
            .replace("reference_loads", '"reference_loads"')
            .replace("to_test_nodes", '"to_test_nodes"')
            .replace("to_test_loads", '"to_test_loads"')
        )
...
def read_json_statistics():
    ...
    with open("results//statistics.json") as file:
        ...
        statistics = Statistics(
            json_object["total_tests_number"],
            json_object["success_percentage"],
            json_object["processing_time"],
        )
    return statistics
...
def read_json_test_case(model_name):
    ...
    with open(f"results//{model_name}.json") as file:
        json_object = json.load(file)
        testcase = TestCase(
            json_object["test_result"],
            json_object["processing_time"],
            json_object["test_comparison"],
        )
    return testcase
